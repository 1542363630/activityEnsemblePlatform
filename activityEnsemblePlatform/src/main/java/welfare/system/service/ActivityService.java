...
public class ActivityService {
    ...
    public Response selectActivityByUserId(Integer uid, Integer page, Integer pageSize) {
        try {
            ...
            // 得到指定页
            List<Map<String, Object>> activityInfoList = MAPPER.activity.selectActivityByUserId(uid, pageSize,
                    (page - 1) * pageSize);
            ...
            return Response.success(
                    new PageResultData<>(maxPage, totalNum, activityInfoList).toReturnMapExceptClassificationList());
        } catch (RuntimeException e) {
            System.out.println(e.getMessage());
    ...
    public Response selectActivityConstrained(ActivitySearchData activitySearchData) {
        ...
        try {
            if (pageData.getNum() == null)
                pageData.setNum(5);
            if (pageData.getPage() <= 0)
                return Response.failure(404, "页数只能为正数");
            ...
            List<Map<String, Object>> activityList = MAPPER.activity.selectActivityConstrained(sql, pageData.getNum(),
                    (pageData.getPage() - 1) * pageData.getNum());
            for (Map<String, Object> m : activityList) {
                m.put("coverURL", VALUE.web_path + VALUE.img_web + m.get("coverURL"));
            }

            return Response.success(
                    new PageResultData<>(maxPage, totalNum, activityList).toReturnMapExceptClassificationList());
        } catch (RuntimeException e) {
            System.out.println(e.getMessage());
    ...
    // MAPPER.activity.selectActivityOngoing(pageSize,(page -1)* pageSize);
    ...
    // MAPPER.activity.selectNumberOfActivityEnd() / pageSize);
    // if(page>maxPage){
    // return Response.failure(404,"页数超过最大页数。最大页数：" + maxPage);
    // }
    // //得到指定页
    // List<ActivityInfo> activityInfoList =
    // MAPPER.activity.selectActivityEnd(pageSize,(page -1)* pageSize);
    // PageResultData<ActivityInfo> pageResultData = new PageResultData<>();
    // pageResultData.setPageNumber(maxPage);
    // pageResultData.setList(activityInfoList);
    //
    ...
    // 申请取消注册活动
    public Response cancelActivity(Integer uid, Integer id) {
        try {
            ActivityInfo activityInfo = MAPPER.activity.selectActivityById(id);
            if (activityInfo == null) {
                return Response.failure(400, "你要取消注册的活动不存在!");
            }
            ...
            if (registerStatus == null || registerStatus != 0) {
                // 未报名
                return Response.failure(400, "你并未报名该活动!");
            }

            // status为0，表明已报名过
            else if (registerStatus == 0) {

                MAPPER.activity.removeActivityRegister(id, uid);
                
                return Response.ok();
            }
            else {
                return Response.failure(401, "奇怪的情况哦");
            }
        } catch (RuntimeException e) {
            System.out.println(e.getMessage());
            e.getStackTrace();
